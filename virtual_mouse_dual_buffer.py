...
WINDOW_NAME = "Virtual Mouse: q(exit), w(hide/show stats), e(zero dist. dragging)"
pyautogui.PAUSE = 0
...
def click_mechanics(index_thumb_distance):
    ...
    if index_thumb_distance < click_distance and click_status == 0:
        click_status = 1
        pyautogui.mouseDown()
        # print(F"click dist: {index_thumb_distance}")
        return

    elif index_thumb_distance > 100:
        click_status = 0
        pyautogui.mouseUp()
...
async def main():
    ...
    while True:
        ...
        if hands:
                ...
                for id, landmark in enumerate(landmarks):
                    ...
                    if id == 4: 
                        ...
                        if zero_distance_dragging == True:
                            ...
                            dragging_threshold = 130
                            
                        if index_thumb_distance > dragging_threshold:
            ...
            # Trigger click mechanics WHEN THERE IS LITTLE ACCELERATION (Holds clicks when dragging)
        ...
        if show_window:
            ...
            if show_stats == True:
                ...
                cv2.putText(frame, f"Aceleration (relative unit/2s): {round((relative_lower_palm_aceleration_display),2)}", (int(50//global_scale), int(230/global_scale)),
                            cv2.FONT_HERSHEY_PLAIN, 3/global_scale, color, text_size, cv2.LINE_AA)
                cv2.putText(frame, f"Zero dist. dragging: {zero_distance_dragging}", (int(50//global_scale), int(280/global_scale)),
                            cv2.FONT_HERSHEY_PLAIN, 3/global_scale, color, text_size, cv2.LINE_AA)
